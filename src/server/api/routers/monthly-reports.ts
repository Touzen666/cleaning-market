...
import { AIRBNB_COMMISSION_PERCENT, formatPercentLabel, isAirbnbCommissionChannel, sumOtaCommissionBaseByChannel } from "@/lib/ota-commission";
...
function mapReservationToAutoRevenueItem(
    ...
    const totalNights = Math.max(1, countStayNights(stayStart, stayEnd));
    const nightsInThisMonth = countOverlapNights(stayStart, stayEnd, periodStart, periodEnd);
    const amountForThisMonth = amountForNightsInPeriod(totalAmount, totalNights, nightsInThisMonth);
    ...
    return {
        ...
        description: `Rezerwacja - ${reservation.guest ?? "gość"} (podział proporcjonalny)`,
        amount: amountForThisMonth,
        currency: reservation.currency ?? "PLN",
        date: stayStart,
        reservationId: reservation.id,
        isAutoGenerated: true,
        notes: buildStaySplitNote(totalAmount, totalNights, nightsInThisMonth),
    };
}
...
export const monthlyReportsRouter = createTRPCRouter({
    ...
    getSuggestedCommissions: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            const channelsMap = sumOtaCommissionBaseByChannel(
            ...
            const suggestions = Array.from(channelsMap.entries())
                ...
                .map(([channel, totalRevenue]) => ({
                    ...
                    notes: isAirbnbCommissionChannel(channel)
                        ...
                        : `Sugerowana prowizja ${channel} od przychodu brutto rezerwacji z wymeldowaniem w tym miesiącu (zgodnie z wykazem wypłaty).`,
                    isAutoGenerated: false,
                }));
