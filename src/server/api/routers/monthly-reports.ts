...
import { resolveReportChannel, getRecognizedReservationChannel, IDOBOOKING_WIDGET_CHANNEL } from "@/lib/reservation-channel";
...
function mapReservationToAutoRevenueItem(
    reservation: {
        id: number;
        guest: string | null;
        start: Date;
    ...
    const totalAmount = reservation.rateCorrection ?? reservation.paymantValue ?? 0;
    const stayStart = new Date(reservation.start);
    const stayEnd = new Date(reservation.end);
    const inPayoutMonth = checkoutFallsInPeriod(stayEnd, periodStart, periodEnd);
    const amountForThisMonth = inPayoutMonth ? roundPln2(totalAmount) : 0;
    ...
    return {
        ...
        description: `Rezerwacja - ${reservation.guest ?? "gość"}`,
        amount: amountForThisMonth,
        currency: reservation.currency ?? "PLN",
        date: stayStart,
        reservationId: reservation.id,
        isAutoGenerated: true,
        notes: "Pełna kwota rezerwacji. W raporcie miesiąca wymeldowania (jak na FV / wykazie wypłaty OTA).",
    };
}
...
// Helper function to calculate laundry costs: pranie raz na 7 dni = liczba dni w miesiącu / 7 × stawka tygodniowa z karty apartamentu
// month: 1–12 (styczeń = 1), zgodnie z report.month
async function calculateLaundryCosts(
    ...
    const weeklyRate = Number(apartment.weeklyLaundryCost);
    const firstDayOfMonth = new Date(year, month - 1, 1);
    const firstDayOfNextMonth = new Date(year, month, 1);
    const daysInMonth = Math.round((firstDayOfNextMonth.getTime() - firstDayOfMonth.getTime()) / (24 * 60 * 60 * 1000));
    const weeksInMonth = Math.round((daysInMonth / 7) * 100) / 100;
    return Math.round(weeksInMonth * weeklyRate * 100) / 100;
}
...
async function calculateTextileCosts(
    ...
    // Jeśli ustawiono kwotę za rezerwację – używamy jej (liczba rezerwacji × stawka)
    const perReservation = apartment.textileCostPerReservation != null && apartment.textileCostPerReservation > 0;
    if (perReservation) {
        return reservations.length * Number(apartment.textileCostPerReservation);
    }
...
async function recalculateReportSettlement(reportId: string, ctx: RecalculateContext) {
    ...
    try {
        ...
        const totalRevenue = roundPln2(revenueItems.reduce((sum, i) => sum + i.amount, 0));
        const totalExpenses = roundPln2(expenseItems.reduce((sum, i) => sum + i.amount, 0));
        const otaCommissions = roundPln2(commissionItems.reduce((sum, i) => sum + i.amount, 0));
        const netIncome = roundPln2(totalRevenue - totalExpenses - otaCommissions + getParkingProfit(report));
...
export const monthlyReportsRouter = createTRPCRouter({
    ...
    create: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            // Auto-generate revenue items for checkouts in this month
            const revenueItems = reservationsForRevenue
                ...
                .map((reservation) =>
                    mapReservationToAutoRevenueItem(
                        reservation,
                        report.id,
                        startDate,
                        nextMonthStartDate,
            ...
            // Koszty sprzątania, prania i tekstyliów nie są już dodawane automatycznie przy tworzeniu raportu.
            // Użytkownik może je dodać ręcznie przyciskami w sekcji "Wydatki i Prowizje".
    ...
    getById: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            // Sugerowane kwoty dla Szybkiego Dodawania Kosztów (na bazie rezerwacji kończących się w tym miesiącu)
            let suggestedCleaning = 0;
            let suggestedLaundry = 0;
            let suggestedTextile = 0;
            try {
    ...
    getSuggestedCommissions: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            const existingChannels = new Set(
                existingCommissions.map((item) =>
                    (getRecognizedReservationChannel(item.category) ?? item.category).trim().toLowerCase(),
                ),
            );
            ...
            const suggestions = Array.from(channelsMap.entries())
                .filter(([channel]) => {
                    const key = (getRecognizedReservationChannel(channel) ?? channel).trim().toLowerCase();
                    return !existingChannels.has(key);
                })
                .map(([channel, { totalRevenue, lineGrossAmounts }]) => {
                    const isAirbnb = isAirbnbCommissionChannel(channel);
                    const isWidget =
                        getRecognizedReservationChannel(channel) ===
                        IDOBOOKING_WIDGET_CHANNEL;
                    return {
                    type: ReportItemType.COMMISSION,
                    category: channel,
                    ...
                    notes: isAirbnb
                        ? `Sugerowana prowizja Airbnb: ${formatPercentLabel(AIRBNB_COMMISSION_PERCENT)}% + 23% VAT od przychodu z wymeldowaniem w miesiącu raportu.`
                        : isWidget
                            ? "Sugerowana prowizja IdoBooking widget: 12% od przychodu brutto rezerwacji z wymeldowaniem w tym miesiącu."
                            : `Sugerowana prowizja ${channel} od przychodu brutto rezerwacji z wymeldowaniem w tym miesiącu (jak na wykazie wypłaty, zaokrąglane per rezerwacja).`,
                    isAutoGenerated: false,
                    };
                });
    ...
    addLaundryCosts: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            // Calculate laundry costs from apartment settings (month 1–12: dni w miesiącu / 7 × stawka tygodniowa)
            const laundryCostPerWeek = Number(report.apartment.weeklyLaundryCost ?? 120);
            const firstDayOfMonth = new Date(report.year, report.month - 1, 1);
            const firstDayOfNextMonth = new Date(report.year, report.month, 1);
            const daysInMonth = Math.round((firstDayOfNextMonth.getTime() - firstDayOfMonth.getTime()) / (24 * 60 * 60 * 1000));
            const weeksInMonth = Math.round((daysInMonth / 7) * 100) / 100;
            const totalLaundryCost = Math.round(weeksInMonth * laundryCostPerWeek * 100) / 100;
